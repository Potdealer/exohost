...
/// @title ExoHostRegistry
/// @notice Decentralized website naming registry for Net Protocol on Base.
///         Register a human-readable name, point it at onchain storage, get a website.
/// @dev Names are ERC-721 NFTs. The gateway reads this contract to resolve
///      {name}.exohost.xyz → Net Protocol storage → serve HTML.
contract ExoHostRegistry is ERC721, Ownable {
    // ─── Storage ────────────────────────────────────────────────
    ...
    uint256 public constant PRICE_3_CHAR = 0.1 ether;
    uint256 public constant PRICE_4_CHAR = 0.01 ether;
    uint256 public constant PRICE_5_PLUS = 0.001 ether;
    ...
    /// @param route URL path (e.g., "about", "projects/cool-thing")
    /// @param key Net Protocol storage key
    ...
    /// @return storageWallet The wallet holding the site's Net storage
    /// @return homepageKey The storage key for the index page
    ...
    /// @return storageWallet The wallet holding the site's Net storage
    /// @return key The storage key for the requested page
        ...
        returns (address storageWallet, string memory key)
    {
        ...
        // If no specific page key, return empty (gateway can fall back to homepage)
